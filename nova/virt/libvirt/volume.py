...
class LibvirtNFSVolumeDriver(LibvirtBaseVolumeDriver):
    ...
    def _ensure_mounted(self, nfs_export, options=None):
        ...
        LOG.debug(_(nfs_export))
        LOG.debug(_(mount_path))
    ...
    def _mount_nfs(self, mount_path, nfs_share, options=None, ensure=False):
        ...
        nfs_cmd.extend([nfs_share, mount_path]) # UUID
...
class LibvirtXFSVolumeDriver(LibvirtBaseVolumeDriver):
    """Class implements libvirt part of volume driver for NFS."""

    def __init__(self, connection):
        """Create back-end to nfs."""
        super(LibvirtXFSVolumeDriver,
              self).__init__(connection, is_block_dev=False)

    def get_config(self, connection_info, disk_info):
        """Returns xml for libvirt."""
        conf = super(LibvirtXFSVolumeDriver,
                     self).get_config(connection_info, disk_info)
    ...
    def disconnect_volume(self, connection_info, disk_dev):
        ...
        try:
            ...
            if ('device is busy' in exc.message or
                'target is busy' in exc.message):
                LOG.debug("The OCFS share %s is still in use.", export)
            else:
                LOG.exception(_LE("Couldn't unmount the OCFS share %s"), export)
    ...
    def _ensure_mounted(self, nfs_export, options=None):
        ...
        mount_path = os.path.join(CONF.libvirt.nfs_mount_point_base,
                                  utils.get_hash_str(nfs_export))
        if not libvirt_utils.is_mounted(mount_path, nfs_export):
            self._mount_xfs(mount_path, nfs_export, options, ensure=True)
        return mount_path

    def _mount_xfs(self, mount_path, nfs_share, options=None, ensure=False):
        """Mount nfs export to mount path."""
        utils.execute('mkdir', '-p', mount_path)
